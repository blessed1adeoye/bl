...
def contact(request):
	if request.method == 'POST':
		...
		c.save()
		return render(request, 'b/contact.html', {
			'name': name,
			'email': email,
			'navbar': 'cont'
		})
...
def dx(request):
	dis = WeeklyDx.objects.all()
	context = {
		'dis': dis,
		'navbar': 'weekly'
	}
	return render(request, 'b/hi.html', context)

def wkDx(request, pk):
	dx = WeeklyDx.objects.get(id=pk)

	context = {
		'dx': dx,
		'navbar': 'weekly'
	}
	return render(request, 'b/him.html', context)
